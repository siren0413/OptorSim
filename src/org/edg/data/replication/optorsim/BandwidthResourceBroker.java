...
public class BandwidthResourceBroker extends SkelResourceBroker {
	...
	public ComputingElement findCE(GridJob job, Optimisable optor) {
		...
		for( Iterator iCE = gc.freeCEs(); iCE.hasNext();) {
		    ...
		    double bandwidthIndex = optor.getBandwidthBenchmark(lfns, ce, fileFraction);
		    
		    
		    if( bandwidthIndex < bestCost)  {
				bestCost = bandwidthIndex;
				lowestAC.clear();
				lowestAC.add(ce);
		    }
		    else if (bandwidthIndex == bestCost){
		    	lowestAC.add(ce);
		    }
		}
